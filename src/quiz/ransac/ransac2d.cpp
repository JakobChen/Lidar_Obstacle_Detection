...
std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	// 1. calculate the A, B C for Ax + By + c =0; (y1-y2)x + (x2-x1)y +(x1*y2- x2*y1) = 0
		float x1, y1, x2, y2;
	...
	std::cout << "Ransac took " << elapsedTime.count() << "millisecondes" << std::endl;
...
void cross_product(std::vector<float>& vector_a, std::vector<float>& vector_b, std::vector<float>& temp) {
...
int main ()
{
	...
	//(cloud, maxIteration,distanceTollerance))
